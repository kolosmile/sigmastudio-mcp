...
Az in-memory backend és a protokoll/graph/catalog tesztek működnek. A SigmaStudio 4.7 interopot a helyi környezetben ellenőriztük: a szerver DLL példányosítása, UI Automation ablakfelderítés, `LINK`, `COMPILE`, `DOWNLOAD`, valamint a valós `EXPORT_SYSTEM_FILES` alapú live graph extraction sikeres volt. A jelenlegi projekt exportja 28 normalizált vizuális blokkot és 53 kapcsolatot adott vissza; két egymást követő export azonos blokk- és kapcsolat-ID-kat eredményezett. A graph transaction, export-alapú checkpoint/undo rollback, UI status és accessibility Capture observer bekerült. A production `block.setControl` útvonal most a dokumentált `SET_OBJECT_PROPERTY` szerződésen keresztül működik, és csak élő readback + új Capture-bizonyíték után jelez sikert; sikertelen ellenőrzésnél visszaállítást kísérel meg. A Capture-válasz alapból kompakt strukturált JSON: a nagy nyers mezők helyett hossz- és SHA-256-metaadatot ad, a teljes szöveg csak explicit `includeRaw=true` esetén kérhető. A tiszta újraindítás utáni production HIL acceptance újrafuttatása még hátravan, mert a SigmaStudio a helyi fixture megnyitását jelenleg `false` eredménnyel utasítja el. A blokk-hozzáadás/átnevezés továbbra is explicit védelmi kapu mögött marad. A strukturális adapter a bizonyítottan 1-alapú exportált `P<n>` indexeket a szerver dokumentált 0-alapú pin-indexeire fordítja; a strukturális HIL acceptance azonban még olyan fixture-t igényel, amelyben a köztes gráf érvényes marad.
