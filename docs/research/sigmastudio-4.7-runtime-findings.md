...
- The local build contains the previously verified Capture observer and clipboard fallback, but Capture is currently disabled at the Bridge/runtime boundary and is not used by snapshots or mutations.
...
- A disposable-project-only `sigma_property_probe_set_control_value` developer probe is available behind the HIL gate. On the local `.hil.dspproj`, the verified sequence was SET `Gain1.Gain` to `0.25`, read-after-write `0.25`, and restore of the original value. The probe requires `SIGMASTUDIO_MCP_HIL_MUTATION=1`, an exact `SIGMASTUDIO_MCP_HIL_PROJECT` match, and a `.hil.dspproj` or `tests/hil-projects` path.
- The production `block.setControl` and `block.setControls` runtime paths now use the verified three-argument `SET_OBJECT_PROPERTY` contract, refresh the live control state, require typed live readback, and attempt restoration when verification fails. Capture observation is retained as disabled optional code and is not a mutation success gate. The production path is unit-tested against the in-memory backend; a clean-restart SigmaStudio HIL acceptance rerun is still pending because the local fixture currently fails to reopen in SigmaStudio.
...
- The documented `CONNECT_OBJECT`/`DISCONNECT_OBJECT` methods are present. The exported NetList exposes one-based `P<n>` graph labels, while the server contract is zero-based; the Bridge now converts at this adapter boundary, and a disposable ADAU1701 stereo Input → Output probe confirmed that `P1/P2` must be sent as server indices `0/1`. The probe also showed that disconnecting a mandatory ADAU1701 output edge makes `COMPILE`/`EXPORT_SYSTEM_FILES` fail, so structural disconnect/reconnect and save-close-reopen acceptance still require a fixture with a valid intermediate graph.
